...
def c2eb_entry(ctx):
    ...
    CEitm_strike_price = get_nifty_strike_by_premium(ctx, "CE", ctx.c2eb_cp)
    ...
    main_leg = {
        ...
        "tag": f"C2EB_{time.strftime('%H%M%S')}"}
...
def c2eb_exit(ctx, open_positions):
    """
    Indicator exit for C2EB
    """
    ...
    # flip B <-> S
    exit_side = "S" if c2eb["trnsTp"] == "B" else "B"

    ltp = get_ltp(ctx, c2eb["tok"])
    if ltp is None:
        logging.error("LTP is None — skipping..")
        return None 
    buffer = ltp * ctx.limitorder
    ...
    logging.info(f"Placing order for {c2eb['trdSym'][-7:]} @ {price}")

    return place_exit_without_sl(
        ctx=ctx,
        main_leg=main_leg,
        exit_tag=f"IND_C2EB_{time.strftime('%H%M%S')}",
        exit_side=exit_side,
        price = price)
