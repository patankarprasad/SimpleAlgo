...
from utility.reconcile import nhf_build_positions
from utility.common_functions import safe_execute, send_telegram_message


def nhf_entry(ctx):
    """
    If today expiry day, then NHF position for next expiry
    Else near expiry
    """
    ...
    # main_h_strike = synthfut_strike_price + (ctx.nhf_hdg_dist)
    ...
    if not main_s_inst or not main_b_inst:# or not main_h_inst:
        logging.info("NHF: Options not found")
        return None
    ...
    if ctx.nearest_nifty_expiry_date == today:
        main_s_leg = { "trdSym": main_s_inst["trdSym"], "tok": main_s_inst["tok"], "side": "S", "qty": ctx.nhf_next_lot, "productType": "NRML", "tag": f"NHF_PE_{int(time.time())}"}
        # main_h_leg = { "trdSym": main_h_inst["trdSym"], "tok": main_h_inst["tok"], "side": "B", "qty": ctx.nhf_next_lot, "productType": "NRML", "tag": f"NHF_HPE_{int(time.time())}"}
        main_b_leg = { "trdSym": main_b_inst["trdSym"], "tok": main_b_inst["tok"], "side": "B", "qty": ctx.nhf_next_lot, "productType": "NRML", "tag": f"NHF_CE_{int(time.time())}" }
    else:
        main_s_leg = { "trdSym": main_s_inst["trdSym"], "tok": main_s_inst["tok"], "side": "S", "qty": ctx.nhf_near_lot, "productType": "NRML", "tag": f"NHF_PE_{int(time.time())}"}
        # main_h_leg = { "trdSym": main_h_inst["trdSym"], "tok": main_h_inst["tok"], "side": "B", "qty": ctx.nhf_near_lot, "productType": "NRML", "tag": f"NHF_HPE_{int(time.time())}"}
        main_b_leg = { "trdSym": main_b_inst["trdSym"], "tok": main_b_inst["tok"], "side": "B", "qty": ctx.nhf_near_lot, "productType": "NRML", "tag": f"NHF_CE_{int(time.time())}" }
    ...
    logging.info(f"Placing order for | CE {main_b_inst['trdSym'][-7:]} | PE {main_s_inst['trdSym'][-7:]}")
...
def nhf_exit(ctx, nhf_open_positions):
    """
    Indicator exit for RCE (main + hedge)
    """

    main_b = nhf_open_positions.get("NHF_CE")
    # main_h = nhf_open_positions.get("NHF_HPE")
    main_s = nhf_open_positions.get("NHF_PE")

    # if not main_b or not main_s:
    #     logging.error("NHF exit requested but one or more legs missing")
    #     return None
    ...
    return place_synthfut_exit_without_hedge(
        ...
        main_s_exit_tag=f"IND_NHF_PE_{int(time.time())}",
        main_b_exit_side=main_b_exit_side,
        main_b_exit_tag=f"IND_NHF_CE_{int(time.time())}",
        buyprice=buyprice,
        sellprice=sellprice)

def rollover_nhf(ctx):
    
    # applicable where we buy synthetic future of next expiry
    
    """
    NHF Rollover:
    If NHF position is open and its expiry is today:
    ...
    if nhf_open_positions is None:
        logging.info("NHF No open NHF position found..\n")
        return False

    # All legs have same expiry, pick any
    exp_dt_str = nhf_open_positions["NHF_CE"]["expDt"]
    ...
    if today != exp_date :
        logging.info(f"NHF Open position found but no need of rollover today, expiry={exp_date}, today={today}")
        return False

    logging.warning(f"NHF ROLLOVER ALERT!!: Expiry today ({exp_date}) hence rolling to next expiry")

    # 1. Force EXIT of current expiry
    nhf_exit(ctx, nhf_open_positions)

    # Give broker + ledger some time to update
    time.sleep(1)

    # 2. Reconcile again to ensure position is closed
    nhf_after_exit = nhf_build_positions(ctx)
    ...
    nhf_reentry = nhf_build_positions(ctx)
    ...
    logging.info("NHF ROLLOVER: New NHF position opened for next expiry..\n")
