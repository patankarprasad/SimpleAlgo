"""
backtest_2ind_v1.py — 2-indicator strategy: Supertrend(10,2) + MA(50).
                       ORIGINAL version — signal-priority based exits.

KEY BEHAVIOUR (different from backtest_2ind.py):
  Exit long  only when signal == "EXIT_LONG"  (close < ST  AND  close > MA)
  Exit short only when signal == "EXIT_SHORT" (close > ST  AND  close < MA)

  Consequence: when price is below BOTH ST and MA, signal = "SELL" takes
  np.select priority over "EXIT_LONG", so an open LONG stays open until
  price recovers above MA (giving EXIT_LONG a chance to fire).
  This naturally filters out whipsaw exits during strong downtrends and
  produces fewer but higher-quality trades for index instruments (NIFTY,
  BANKNIFTY) where trends are cleaner.

Usage:
    python backtest_2ind_v1.py                               # all 15-min instruments, 6 months
    python backtest_2ind_v1.py --months 3
    python backtest_2ind_v1.py --instruments NIFTY,BANKNIFTY
    python backtest_2ind_v1.py --no-save
"""
...
from indicators import supertrend, sma
...
logger = logging.getLogger("backtest_2ind_v1")
...
def compute_signals_2ind(df: pd.DataFrame) -> pd.DataFrame:
    """
    Signals — np.select priority (first match wins):
      1. BUY        — close > ST  AND  close > MA
      2. SELL       — close < ST  AND  close < MA   ← masks EXIT_LONG in downtrends
      3. EXIT_LONG  — close < ST  (only fires when close > MA)
      4. EXIT_SHORT — close > ST  (only fires when close < MA)
    """
    df = df.copy()
...
# ══════════════════════════════════════════════════════════════════════════════
# Strategy simulation  — signal-string based exits (original v1 behaviour)
# ══════════════════════════════════════════════════════════════════════════════
...
def simulate_strategy(inst_cfg: dict, df: pd.DataFrame) -> list[dict]:
    ...
    for idx, (ts, row) in enumerate(df_s.iterrows()):
        ...
        # ── Signal processing (signal-string exits — v1 behaviour) ─────────
        if position == 0:
            ...
            # EXIT_LONG only fires when close < ST but close > MA
            # (SELL has higher priority when close < ST AND close < MA,
            #  so the long rides through deep downtrends until MA is reclaimed)
            if signal == "EXIT_LONG":
...
def print_report(results: dict, from_date: date, to_date: date):
    ...
    print(f"\n{WIDE}")
    print(f"  BACKTEST REPORT (2-IND v1 — signal-priority exits)   |   Period: {from_date}  →  {to_date}")
    print(f"  Strategy: Supertrend({ST_PERIOD}/{ST_FACTOR}) + SMA({MA_LENGTH})   "
          f"|  Exit: EXIT_LONG signal only (SELL masks exit in deep downtrends)")
    print(WIDE)
...
def main():
    parser = argparse.ArgumentParser(description="Backtest 2-ind v1: ST(10,2)+MA(50), signal-priority exits")
    parser.add_argument("--months",      type=int,   default=6,  help="Lookback in months (default: 6)")
    parser.add_argument("--instruments", type=str,   default="", help="Comma-separated names e.g. NIFTY,BANKNIFTY")
    ...
    logger.info("Strategy: ST(%d/%.1f) + MA(%d)  |  v1 signal-priority exits",
                ST_PERIOD, ST_FACTOR, MA_LENGTH)
    logger.info("═" * 60)
    ...
    if not args.no_save:
        ...
        if all_trades:
            out_path = f"backtest_2ind_v1_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
