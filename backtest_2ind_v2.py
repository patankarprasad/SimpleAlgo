"""
backtest_2ind_v2.py — 2-indicator strategy: Supertrend(10,3) + MA(50).

Strategy:
  Long  Entry : close > ST(10,3)  AND  close > MA(50)
  Long  Exit  : close < ST(10,3)        ← stoploss, no MA filter
  Short Entry : close < ST(10,3)  AND  close < MA(50)
  Short Exit  : close > ST(10,3)        ← stoploss, no MA filter

Exit uses raw indicator value (not signal string) so there is no
np.select priority conflict — long exits cleanly the moment price
closes below ST(10,3), regardless of MA position.

Usage:
    python backtest_2ind_v2.py                               # all 15-min instruments, 6 months
    python backtest_2ind_v2.py --months 3
    python backtest_2ind_v2.py --instruments GOLDM,NIFTY
    python backtest_2ind_v2.py --hourly
    python backtest_2ind_v2.py --all
    python backtest_2ind_v2.py --no-save
"""
...
def compute_signals(df: pd.DataFrame) -> pd.DataFrame:
    """
    Signals:
      BUY        — close > ST(10,3)  AND  close > MA(50)
      SELL       — close < ST(10,3)  AND  close < MA(50)
      EXIT_LONG  — close < ST(10,3)   (used for reference only; simulation exits on raw value)
      EXIT_SHORT — close > ST(10,3)   (used for reference only; simulation exits on raw value)
    """
    df = df.copy()
    df["st"] = supertrend(df, ST_PERIOD, ST_FACTOR)
...
def simulate_strategy(inst_cfg: dict, df: pd.DataFrame) -> list[dict]:
    ...
    for idx, (ts, row) in enumerate(df_s.iterrows()):
        ...
        # ── Signal processing ──────────────────────────────────────────────
        # Exits check raw ST value so a long is always closed the moment
        # close < ST(10,3), regardless of what np.select chose as the signal.
        ...
        if position == 0:
            ...
            if below_st:                            # stoploss: close < ST(10,3)
                ...
                entry_ts = None
                entry_exp = None
                # Flip to short if full SELL signal is active
            ...
            if above_st:                            # stoploss: close > ST(10,3)
                ...
                entry_ts = None
                entry_exp = None
                # Flip to long if full BUY signal is active
                if in_hrs and signal == "BUY":
                    position  = +1
                    entry_px  = close
                    entry_ts  = ts
...
def print_report(results: dict, from_date: date, to_date: date):
    ...
    print(f"\n{WIDE}")
    print(f"  BACKTEST REPORT (2-IND v2)   |   Period: {from_date}  →  {to_date}")
...
def main():
    parser = argparse.ArgumentParser(description="Backtest 2-ind v2: ST(10,3)+MA(50), raw-value exits")
    ...
    if not args.no_save:
        ...
        if all_trades:
            out_path = f"backtest_2ind_v2_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
